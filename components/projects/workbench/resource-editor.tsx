    ...
    <SectionCard title="Resource definition" description="Resources are aggregated into the economy model for the MVP simulation engine.">
      {workingVersion.resources.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-white/15 bg-slate-950/40 p-5">
          <h3 className="text-lg font-semibold text-white">No resources defined yet</h3>
          <p className="mt-3 text-sm leading-7 text-slate-400">
            Add at least one resource so the simulation engine can model how players pay for cards over time. You can fine-tune starting amounts and per-turn gain later.
          </p>
          <button
            type="button"
      ...
      <button type="button" onClick={handleAddResource} className="mt-4 inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-2 text-sm text-white transition hover:border-cyan-400/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/50">
        <Plus className="h-4 w-4" />
        {workingVersion.resources.length === 0 ? "Add starter resource" : "Add resource"}
      </button>
    </SectionCard>
  );
}
